...
function [im_final_w,im_final,edge_w,edge]=SPEN_recon_SJTU_mat(mat,SPENpara)
...
im_h1 = flip(squeeze(sos(cifftn(ksp_rec_1,[1,2]),3)),1);
...
im_h3 = flip(squeeze(sos(cifftn(ksp_rec_3,[1,2]),3)),1);
...
im_final_w=imresize(cifftn(bsxfun(@times,cfftn(im_final,[1 2]),winf),[1 2]),[Npe*2,Nx*2],'lanczos2');
edge_w=imresize(cifftn(bsxfun(@times,cfftn(edge,[1 2]),winf),[1 2]),[Npe*2,Nx*2],'lanczos2');
disp('done.')
